...
module.exports.seguridad = function (app) {
	...
	app.route('/api/usuarios')
		.post(function (req, res, next) {
			...
			usuariosData.gettingByEmail(usuario.email)
				.then(function (data) {
					if (data) {
	...
	app.route('/api/sesiones')
		.post(function (req, res, next) {
			var usuario = req.body;
			usuariosData.gettingByEmailPassword(usuario.email, usuario.password)
				.then(function (data) {
					if (data) {
						console.log('aceptado:' + usuario.email);
						res.json(newSession(usuario.email));
					} else {
						console.error('Credencial inválida:' + usuario.email);
						res.status(401).send('Credencial inválida');
					}
				})
				.fail(function (err) {
